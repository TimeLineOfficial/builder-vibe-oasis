...
export default function LatestJobs() {
  ...
  // Get unique sectors
  const sectors = ['all', ...Array.from(new Set(allJobs.map(job => job.sector)))];
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      jobs = searchJobs(searchQuery).filter(job => 
  ...
  const JobCard = ({ job, onClick }: { job: JobPosting; onClick: () => void }) => {
    ...
    const isExpiring = timeRemaining.includes('day') && !timeRemaining.includes('days');
        ...
        <CardContent className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-gray-700 font-medium">{job.organization}</span>
              </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-semibold text-green-600">{job.salary.total_range}</span>
            </div>
  ...
  const JobDetailsModal = ({ job, onClose }: { job: JobPosting; onClose: () => void }) => (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex gap-2 mt-2">
                <Badge variant={job.type === 'government' ? 'default' : 'secondary'}>
                  {job.type === 'government' ? 'Government' : 'Private'}
                </Badge>
            ...
            <TabsContent value="overview" className="mt-6">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div>
                      <span className="text-sm text-gray-600">Application Start:</span>
                      <p className="font-semibold">{formatDate(job.start_date)}</p>
                    </div>
                    <div>
                      <span className="text-sm text-gray-600">Application End:</span>
                      <p className="font-semibold">{formatDate(job.end_date)}</p>
                    </div>
                    <div>
                      <span className="text-sm text-gray-600">Time Remaining:</span>
                      <p className={`font-semibold ${
                        getTimeRemaining(job.end_date).includes('Expired') ? 'text-red-600' :
                        getTimeRemaining(job.end_date).includes('day') && !getTimeRemaining(job.end_date).includes('days') ? 'text-orange-600' :
                        'text-green-600'
                      }`}>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">{job.salary.basic_pay}</p>
                  </div>
                  
                  {job.salary.allowances && job.salary.allowances.length > 0 && (
                    <div>
                      <span className="text-sm text-gray-600">Allowances:</span>
                      <div className="flex flex-wrap gap-2 mt-1">
                        {job.salary.allowances.map((allowance, i) => (
                          <Badge key={i} variant="outline">{allowance}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-blue-600">{job.salary.total_range}</p>
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex gap-4">
                      ...
                      {job.form_guide_youtube && (
                        <Button variant="outline" className="gap-2" asChild>
                          <a href={job.form_guide_youtube} target="_blank" rel="noopener noreferrer">
                    ...
                    <div className="p-4 bg-blue-50 rounded-lg">
                      ...
                      <ul className="text-sm text-blue-800 space-y-1">
                        <li>• Fill the form carefully and double-check all details</li>
                        <li>• Keep all required documents ready in PDF format</li>
                        <li>• Apply well before the deadline</li>
                        <li>• Take a screenshot of the application form after submission</li>
                        <li>• Keep the registration number safe for future reference</li>
                      </ul>
                    </div>
                  </CardContent>
                </Card>
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">
            {getText('latest_jobs_title', currentLanguage) || 'Latest Job Vacancies'}
          </h1>
          <p className="text-xl text-gray-600">
            {getText('latest_jobs_subtitle', currentLanguage) || '100 verified job opportunities updated daily'}
          </p>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <Tabs value={selectedType} onValueChange={(value) => setSelectedType(value as any)}>
                <TabsList>
                  ...
                  <TabsTrigger value="government">Government ({governmentJobs.length})</TabsTrigger>
                  <TabsTrigger value="private">Private ({privateJobs.length})</TabsTrigger>
                </TabsList>
              </Tabs>
